  ...
  if(sess === undefined){
    ...
    if(sess.user_type !='HOD' && sess.user_type !='A' && sess.user_type !='DT' && sess.user_type !='DL'
      ...
      return res.redirect('/login');
       ...
       var prdSQL="select a.PRODUCT_ID,a.NAME,a.DESCRIPTION,a.THUMBNAIL,a.FULLIMAGE,c.INVENTORY,c.LISTPRICE,c.OFFERPRICE from product a join "+
       "productconf c on c.PRODUCT_ID=a.PRODUCT_ID WHERE a.PUBLISHED=1";
      ...
      db.query(GETPERFORMANCE, function(err, result) {
        ...
        db.query(dltSQL, function(err, result2) {
          ...
          db.query(seSQL, function(err, result3) {
            ...
            res.render('_sales_target',{ dldtList:result2,seList:result3,salestargets:result,userId:sess.user_id,userType:sess.user_type,username:sess.username,userImg:sess.user_img, error:''});
          });
        });
      });
    }else{
...
router.post('/add_sales_target', function(req, res, next) {
  ...
  var sales_target_tenure = req.body.sales_target_tenure;
  var sales_target_year = req.body.sales_target_year;
  var sales_target_amount = req.body.sales_target_amount;
  var one_month_target = 0.00;
  if(sess === undefined){
    return res.redirect('/login');
  }else{
  //quarter sales target
  var check_sales_target = "SELECT * FROM sales_target where user_id="+seid+" AND SALES_TARGET_STATUS = 0";

  db.query(check_sales_target, function(err, se){
    if(se.length > 0){
      var GETPERFORMANCE = "SELECT * FROM sales_target";
      var dltSQL="select a.USER_ID,a.LOGON_ID1,d.FIRSTNAME,d.LASTNAME from user a join usrrole b on b.USER_ID=a.USER_ID"+
            " join role c on c.ROLE_ID=b.ROLE_ID join address d on d.MEMBER_ID=a.USER_ID "+
            ...
            db.query(GETPERFORMANCE, function(err, result) {
              ...
              db.query(dltSQL, function(err, result2) {
                ...
                db.query(seSQL, function(err, result3) {
                  ...
                  res.render('_sales_target',{ dldtList:result2,seList:result3,salestargets:result,userId:sess.user_id,userType:sess.user_type,username:sess.username,userImg:sess.user_img, error: "Sales Executive Target still Active, kindly close the target and try again!"});
                });
              });
            });

    }else{
      if(sales_target_tenure === 'Quarter'){
        one_month_target = (sales_target_amount/3).toFixed(2);
        user_id = seid;
            var ApprovedOrderSQLApr="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
                " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
                " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
                " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 04 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLMay="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 05 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLJun="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 06 and YEAR(a.CREATETIME)=2021 ;";
            var finalQuery = ApprovedOrderSQLApr+ApprovedOrderSQLMay+ApprovedOrderSQLJun;
            db.query(finalQuery,function(err, result) {
              ...
              console.log(result[0][0].ORDERTOTAL);
              console.log(result[1][0].ORDERTOTAL);
              console.log(result[2][0].ORDERTOTAL);
              var april_sales = result[0][0].ORDERTOTAL;
              var may_sales =result[1][0].ORDERTOTAL;
              var jun_sales = result[2][0].ORDERTOTAL
              ...
              db.query(get_user_logon_id, function(err, users) {
                users.forEach(function(user){
                  ...
                  add_sales_target = "insert into sales_target (TARGET_TENURE, TARGET_YEAR, user_id, USER_LOGIN_ID,SALES_TARGET,SALES_ACHIEVED,SALES_TARGET_STATUS,CREATETIME, UPDATETIME, APR_TARGET, APR_ACHIEVED, MAY_TARGET, MAY_ACHIEVED, JUN_TARGET, JUN_ACHIEVED) values ('3','"+sales_target_year+"','"+user_id+"','"+login_id+"','"+sales_target_amount+"','"+ordertotal+"', 0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+one_month_target+"','"+april_sales+"','"+one_month_target+"','"+may_sales+"','"+one_month_target+"','"+jun_sales+"')";
      ...
      }  //half yearly sales target
      else if(sales_target_tenure === 'Half'){
        one_month_target = (sales_target_amount/6).toFixed(2); 
        user_id = seid;
            var ApprovedOrderSQLApr="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
                " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
                " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            ...
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 06 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLJul="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
                " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
                " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
                " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 07 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLAug="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 08 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLSep="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 09 and YEAR(a.CREATETIME)=2021 ;";
            var finalQuery = ApprovedOrderSQLApr+ApprovedOrderSQLMay+ApprovedOrderSQLJun+ApprovedOrderSQLJul+ApprovedOrderSQLAug+ApprovedOrderSQLSep;
            db.query(finalQuery, function(err, result) {
              if (err) {
                throw err;
                return res.redirect('/login');
              ...
              var may_sales =result[1][0].ORDERTOTAL;
              var jun_sales = result[2][0].ORDERTOTAL
              var jul_sales = result[3][0].ORDERTOTAL;
              var aug_sales = result[4][0].ORDERTOTAL;
              var sep_sales = result[5][0].ORDERTOTAL;
                ...
                db.query(get_user_logon_id, function(err, users) {
                  users.forEach(function(user){
                    var login_id  = user.LOGON_ID1;
                    add_sales_target = "insert into sales_target (TARGET_TENURE, TARGET_YEAR, user_id, USER_LOGIN_ID,SALES_TARGET,SALES_ACHIEVED,SALES_TARGET_STATUS,CREATETIME, UPDATETIME, APR_TARGET, APR_ACHIEVED, MAY_TARGET, MAY_ACHIEVED, JUN_TARGET, JUN_ACHIEVED, JUL_TARGET, JUL_ACHIEVED, AUG_TARGET, AUG_ACHIEVED, SEP_TARGET, SEP_ACHIEVED) values ('6','"+sales_target_year+"','"+user_id+"','"+login_id+"','"+sales_target_amount+"','"+ordertotal+"', 0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+one_month_target+"','"+april_sales+"','"+one_month_target+"','"+may_sales+"','"+one_month_target+"','"+jun_sales+"','"+one_month_target+"','"+jul_sales+"','"+one_month_target+"','"+aug_sales+"','"+one_month_target+"','"+sep_sales+"')";
                    db.query(add_sales_target, function(err, result) {
                      if (err) {
                        throw err;
                        return res.redirect('/login');
        ...
        one_month_target = (sales_target_amount/12).toFixed(2); 
        user_id = seid;
        var ApprovedOrderSQLApr="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
                " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
                " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            ...
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 09 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLOct="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 10 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLNov="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 11 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLDec="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 12 and YEAR(a.CREATETIME)=2021 ;";
            var ApprovedOrderSQLJan="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 01 and YEAR(a.CREATETIME)=2022 ;";
            var ApprovedOrderSQLFeb="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 02 and YEAR(a.CREATETIME)=2022 ;";
            var ApprovedOrderSQLMar="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
            " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
            " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
            " where a.member_id="+seid+" and a.STATUS='A' and MONTH(a.CREATETIME) = 03 and YEAR(a.CREATETIME)=2022 ;";
            var finalQuery = ApprovedOrderSQLApr+ApprovedOrderSQLMay+ApprovedOrderSQLJun+ApprovedOrderSQLJul+ApprovedOrderSQLAug+ApprovedOrderSQLSep+ApprovedOrderSQLOct+ApprovedOrderSQLNov+ApprovedOrderSQLDec+ApprovedOrderSQLJan+ApprovedOrderSQLFeb+ApprovedOrderSQLMar;
            db.query(finalQuery, function(err, result) {
              if (err) {
                throw err;
                return res.redirect('/login');
              ...
              var aug_sales = result[4][0].ORDERTOTAL;
              var sep_sales = result[5][0].ORDERTOTAL;
              var oct_sales = result[6][0].ORDERTOTAL;
              var nov_sales = result[7][0].ORDERTOTAL;
              var dec_sales = result[8][0].ORDERTOTAL;
              var jan_sales = result[9][0].ORDERTOTAL;
              var feb_sales = result[10][0].ORDERTOTAL;
              var mar_sales = result[11][0].ORDERTOTAL;
                ...
                db.query(get_user_logon_id, function(err, users) {
                  users.forEach(function(user){
                    var login_id  = user.LOGON_ID1;
                    add_sales_target = "insert into sales_target (TARGET_TENURE, TARGET_YEAR, user_id, USER_LOGIN_ID,SALES_TARGET,SALES_ACHIEVED,SALES_TARGET_STATUS,CREATETIME, UPDATETIME, APR_TARGET, APR_ACHIEVED, MAY_TARGET, MAY_ACHIEVED, JUN_TARGET, JUN_ACHIEVED, JUL_TARGET, JUL_ACHIEVED, AUG_TARGET, AUG_ACHIEVED, SEP_TARGET, SEP_ACHIEVED, OCT_TARGET, OCT_ACHIEVED, NOV_TARGET, NOV_ACHIEVED, DEC_TARGET, DEC_ACHIEVED, JAN_TARGET, JAN_ACHIEVED, FEB_TARGET, FEB_ACHIEVED, MAR_TARGET, MAR_ACHIEVED) values ('12','"+sales_target_year+"','"+user_id+"','"+login_id+"','"+sales_target_amount+"','"+ordertotal+"', 0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+one_month_target+"','"+april_sales+"','"+one_month_target+"','"+may_sales+"','"+one_month_target+"','"+jun_sales+"','"+one_month_target+"','"+jul_sales+"','"+one_month_target+"','"+aug_sales+"','"+one_month_target+"','"+sep_sales+"','"+one_month_target+"','"+oct_sales+"','"+one_month_target+"','"+nov_sales+"','"+one_month_target+"','"+dec_sales+"','"+one_month_target+"','"+jan_sales+"','"+one_month_target+"','"+feb_sales+"','"+one_month_target+"','"+mar_sales+"')";
                    db.query(add_sales_target, function(err, result) {
                      if (err) {
                        throw err;
                        return res.redirect('/login');
...
router.get('/productdtl/:id', function(req, res, next) {
  ...
  if(sess === undefined){
    ...
    if(sess.user_type !='HOD' && sess.user_type !='A' && sess.user_type !='DT' && sess.user_type !='DL'
      ...
      var prdSQL="select a.PRODUCT_ID,a.NAME,a.DESCRIPTION,a.THUMBNAIL,a.FULLIMAGE,c.INVENTORY,c.LISTPRICE,c.OFFERPRICE from product a "+
      " join productconf c on c.PRODUCT_ID=a.PRODUCT_ID "+
      " WHERE a.PUBLISHED=1 and a.PRODUCT_ID="+prdId+"";
       ...
       db.query(prdSQL, function(err,result) {
           ...
           var spareSQL="select * from pspare where STATUS=1";
    ...
    return res.redirect('/');
      ...
      if(spid == undefined)
      {
          var c_count=1;
            ...
            " '"+stotal+"',(select GROUP_CONCAT(concat(PSPARE_ID,'/',PRICE) SEPARATOR ',') from pspare where PSPARE_ID IN ("+spid+")),'"+ftotalwithTax+"')";
...
router.post('/upload_receipt', function(req, res, next) {
  ...
  const form = new formidable.IncomingForm();
    form.parse(req, function(err, fields, files){
        ...
        var newPath = "./public/lr_receipts/"+orderId+'.jpg';
                                                ...
                                                var user_id = memid;
                                                  if(rl === 'SE'){
                                                    ...
                                                    var ApprovedOrderSQL="select COUNT(a.ORDER_ID) AS ORDERCOUNT,(CASE WHEN SUM(a.TOTAL) IS NULL THEN 0 ELSE SUM(a.TOTAL) END) AS ORDERTOTAL from orders a "+
                                                        " join address c on c.address_id=a.address_id and c.member_id=a.member_id_for"+
                                                        " join user d on d.USER_ID=a.member_id_for join address e on e.member_id=d.USER_ID "+
                                                        " where a.member_id="+user_id+" and a.STATUS='A' and MONTH(a.UPDATETIME) = "+month+" and YEAR(a.UPDATETIME) = "+year+"";
                                                      if(month == "04"){
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", APR_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", MAY_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", JUN_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", JUL_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", AUG_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", SEP_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", OCT_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", NOV_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
                                                        ...
                                                        db.query(ApprovedOrderSQL, function(err, result) {
                                                          if (err) {
                                                            ...
                                                            ordertotal=result[0].ORDERTOTAL;
                                                            update_sales_target = "update sales_target set SALES_ACHIEVED ="+ordertotal+", DEC_ACHIEVED ="+ordertotal+" where SALES_TARGET_STATUS = 0 AND user_id="+user_id+"";
                                                            db.query(update_sales_target, function(err, result) {
                                                              if (err) {
                                                                throw err;
                                                                return res.redirect('/orders');
...
router.get('/deleteCartItem/cartId/:cartId/cartDtlId/:cartDtlId', function(req, res, next) {
  ...
  router.post('/product_cr_post', function(req, res, next) {
    ...
    if(sess === undefined){
      ...
      if(sess.user_type !='HOD' && sess.user_type !='A' && sess.user_type !='DT' && sess.user_type !='DL'
        ...
        var item_no = randomString();
         var prdCrSQL="insert into product(PRODUCT_TYPE,NAME,DESCRIPTION,PUBLISHED,THUMBNAIL,FULLIMAGE, ITEMNO)"+
         " value('SKU','"+name+"','"+desc+"','1','"+pimage+"','"+pimage+"','"+item_no+"')";
  ...
  router.get('/update_user_dl/id/:id/st/:st', function(req, res, next) {
    var sess=req.session;
    var id=req.params.id;
    var status=req.params.st;
    if(sess === undefined){
  ...
  router.get('/update_user_dt/id/:id/st/:st', function(req, res, next) {
    var sess=req.session;
    var id=req.params.id;
    var status=req.params.st;
    if(sess === undefined){
    ...
    return res.redirect('/login');
      ...
      if(sess.user_type !='HOD' && sess.user_type !='A' && sess.user_type !='DT' && sess.user_type !='DL'
        ...
        return res.redirect('/login');
          ...
          var get_mbm_details_query = "SELECT * FROM mbm_details WHERE id = 1";
                ...
                db.query(prd_sql, function(err,result3) {
                    ...
                    db.query(ord_sql, function(err,result4) {
                        ...
                        db.query(enq_sel_sql, function(err,result5) {
                          ...
                          db.query(get_mbm_details_query, function(err, result6) {
                            if (err) {
                              throw err;
                              return err;
                            }
                            res.render('_invoice',{address:result2,products:result3,ordersdtl:result4,invoice:result5,mbm_details:result6,moment:moment,userId:req.params.user_id,userType:req.params.user_type,username:req.params.username,userImg:req.params.user_img});
                          });
                        });
                      });
                  });
                    ...
                    db.query(ord_sql, function(err,result4) {
                        ...
                        db.query(enq_sel_sql, function(err,result5) {
                            ...
                            db.query(get_mbm_details_query, function(err, result6) {
                              ...
                              res.render('_invoicemobile',{address:result2,products:result3,ordersdtl:result4,invoice:result5,mbm_details:result6,moment:moment,userId:req.params.user_id,userType:req.params.user_type,username:req.params.username,userImg:req.params.user_img});
                            });
                          });
                      });
                  });
...
router.get('/upload_products', function(req, res, next) {
  ...
  if(sess === undefined){
    ...
    var get_all_products = "SELECT * FROM mbm_products";
    db.query(get_all_products, function(err, result1) {
      if(err){
        throw err;
      }
      return res.render('_upload_products',{ products:result1,userId:sess.user_id,userType:sess.user_type,moment:moment,username:sess.username,userImg:sess.user_img, error:""});
    });                               
  }
});
    ...
    form.parse(req, function(err, fields, files){
        ...
        fs.writeFile(newPath, rawData, function(err){
            ...
            readXlsxFile(path).then((rows) => {
              // skip header
              console.log();
              if(rows[0][0] === 'Sub Group Level1' && rows[0][1] === 'Sub Group Level2' && rows[0][2] === 'Item No.' && rows[0][3] === 'Item Description' && rows[0][4] === 'In Stock')
              {
                rows.shift();
                //let products = [];
                rows.forEach((row) => {
                  var sql_command = "INSERT INTO mbm_products (item_no, sub_group_level_1, sub_group_level_2, in_stock, item_description) "+
                  " VALUES ('"+row[2]+"','"+row[0]+"','"+row[1]+"',"+row[4]+",'"+row[3]+"') "+
                  " ON DUPLICATE KEY UPDATE in_stock = "+row[4]+"";
                  db.query(sql_command, function(err,result) {
                    if (err) {
                      throw err;    
                    }
                  });
                ...
                db.query(get_all_products, function(err, result1) {
                  ...
                  return res.render('_upload_products',{ products:result1,userId:sess.user_id,userType:sess.user_type,moment:moment,username:sess.username,userImg:sess.user_img,error: "Invalid xlsx file format!"});
                });
              }
...
/* remove erp product */
router.post('/remove_erp_product', function(req, res, next){
  var sess = req.session;
  var erp_product_id = req.body.erp_product_id;
  var remove_erp_product_query = "DELETE FROM mbm_products WHERE id = '"+erp_product_id+"'";
  db.query(remove_erp_product_query, function(err, result){
    if(err){
      throw err;
    }
    return res.redirect('/upload_products');
...
/* upload product from excel */
router.get('/upload_payments', function(req, res, next) {
  var sess = req.session;
  if(sess === undefined){
    return res.redirect('/login');
  }else{
    var get_all_products = "SELECT * FROM mbm_payments";
    db.query(get_all_products, function(err, result1) {
      if(err){
        throw err;
      }
      return res.render('_upload_payments',{ payments:result1,userId:sess.user_id,userType:sess.user_type,moment:moment,username:sess.username,userImg:sess.user_img, error:""});
    });                               
  }
});

router.post('/upload_payment_file', function(req, res, next) {
  var sess=req.session;
  const form = new formidable.IncomingForm();
    form.parse(req, function(err, fields, files){
        var oldPath = files.productfile.path;
        var newPath = "./public/product_excel/customer_balance_details.xlsx";
        var rawData = fs.readFileSync(oldPath)
        fs.writeFile(newPath, rawData, function(err){
            if(err) console.log(err)
            //return res.redirect('/discount_and_ads');
            var path = "./public/product_excel/customer_balance_details.xlsx";
    
            readXlsxFile(path).then((rows) => {
              // skip header
              console.log();
              if(rows[0][0] === 'Customer/Vendor Code' && rows[0][1] === 'Customer/Vendor Name')
              {
                rows.shift();
                //let products = [];
                rows.forEach((row) => {
                  ...
                  console.log(row[3]+'');
                  console.log(row[4]+'');
                  var sql_command = "INSERT INTO mbm_payments (customer_vendor_code, customer_vendor_name, invoice_number, invoice_date, invoice_amount, paid_amount, due_amount, due_date, credit_days, sales_employee_code, sales_employee_name) "+
                  " VALUES ('"+row[0]+"','"+row[1]+"','"+row[2]+"',"+row[3]+",'"+row[4]+"','"+row[5]+"','"+row[6]+"','"+row[7]+"','"+row[8]+"','"+row[9]+"','"+row[10]+"') "+
                  " ON DUPLICATE KEY UPDATE invoice_number = "+row[2]+"";
                  db.query(sql_command, function(err,result) {
                    if (err) {
                      throw err;    
                    }
                ...
                db.query(get_all_products, function(err, result1) {
                  ...
                  return res.render('_upload_payments',{ payments:result1,userId:sess.user_id,userType:sess.user_type,moment:moment,username:sess.username,userImg:sess.user_img,error: "Invalid xlsx file format!"});
                });
              }
...
/* remove erp product */
router.post('/remove_erp_payment', function(req, res, next){
  var sess = req.session;
  var erp_product_id = req.body.erp_product_id;
  var remove_erp_product_query = "DELETE FROM mbm_payments WHERE id = '"+erp_product_id+"'";
  db.query(remove_erp_product_query, function(err, result){
    if(err){
      throw err;
    }
    return res.redirect('/upload_payments');
  })
});
...
//mbm details 
router.get('/get_mbm_details', function(req, res, next) {
  var sess=req.session;
  
    var get_mbm_details_query = "SELECT * FROM mbm_details WHERE id = 1";
    db.query(get_mbm_details_query, function(err, result) {
      if(err){
        throw err;
      }
      res.render('_mbm_details',{mbm_details:result,userId:sess.user_id,userType:sess.user_type,moment:moment,username:sess.username,userImg:sess.user_img});
    });
  
});

router.post('/update_mbm_details', function(req, res, next) {
  var sess = req.session;
  var mbm_address = req.body.mbm_address;
  var mbm_gstin = req.body.mbm_gstin;
  var mbm_website = req.body.mbm_website;
  var mbm_bank_name = req.body.mbm_bank_name;
  var mbm_ac_number = req.body.mbm_ac_number;
  var mbm_branch_name = req.body.mbm_branch_name;
  var mbm_IFSC_code = req.body.mbm_IFSC_code;
  
    var query = 'UPDATE mbm_details SET mbm_address = ?, mbm_gstin = ?, mbm_website = ?, mbm_bank_name = ?, mbm_ac_number = ?, mbm_branch_name = ?, mbm_IFSC_code = ? WHERE id = 1';
    db.query(query, [mbm_address,mbm_gstin,mbm_website,mbm_bank_name,mbm_ac_number,mbm_branch_name,mbm_IFSC_code], function(err,result) {
      if (err) {
        throw err;
        return res.redirect('/login');
      }
       return res.redirect('/get_mbm_details');
    });
});
...
router.post('/update_products', function(req, res, next) {
  var get_all_products = "SELECT * FROM mbm_products";
  db.query(get_all_products, function(err, results) {
    results.forEach(data => {
      if(data.sub_group_level_1 === 'CHAIR PARTS'){
        var check_spare_parts = "SELECT * FROM pspare WHERE ITEMNO = '"+data.item_no+"'";
        db.query(check_spare_parts, function(err, spareres) {
          console.log(spareres);
          if(spareres.length == 0){
            console.log(data.item_no);
            var insert_into_spare = "INSERT INTO pspare (NAME, DESCRIPTION, STATUS, PRICE, ITEMNO) VALUES(?,?,?,?,?)";
            db.query(insert_into_spare, [data.sub_group_level_2, data.item_description, '1', '200',data.item_no],function(err, spares) {
              if(err){
                throw err;
              }
            });
      ...
      }else if (data.sub_group_level_1 === 'FULL CHAIR'){
        console.log("in full chair");
        var get_product = "SELECT * FROM product WHERE ITEMNO = '"+data.item_no+"'";
          db.query(get_product, function(err, result1) {
            if(err)throw err;
            
            if(result1.length > 0){
              console.log("----item do not exists: "+data.item_no);
              var product_id = result1[0].PRODUCT_ID;
              var update_inventory = "UPDATE productconf SET INVENTORY="+data.in_stock+" WHERE PRODUCT_ID ="+product_id+"";
              db.query(update_inventory, function(err, results){
                if(err)throw err;
              });
            }else{
              console.log("--------is Full Chair");
              console.log(result1);
              var insert_into_spare = "INSERT INTO product (PRODUCT_TYPE, NAME, DESCRIPTION, PUBLISHED, THUMBNAIL, FULLIMAGE, ITEMNO) VALUES(?,?,?,?,?,?,?)";
              db.query(insert_into_spare, ['SKU',data.sub_group_level_2, data.item_description, '1', 'variant_16010339383yX9C4M1bt.jpg','variant_16010339383yX9C4M1bt.jpg',data.item_no],function(err, products) {
                if(err){
                  throw err;
                }
                var product_id = products.insertId;
                var insert_into_config = "INSERT INTO productconf (PRODUCT_ID, INVENTORY, LISTPRICE, OFFERPRICE) VALUES(?, ?, ?, ?)";
                db.query(insert_into_config, [product_id, data.in_stock, '2000', '1500'],function(err, spares) {
                  if(err){
                    throw err;
                  }
                });
      ...
      }else if(data.sub_group_level_1 === 'IMPORT'){
        var get_product = "SELECT * FROM product WHERE ITEMNO = '"+data.item_no+"'";
          db.query(get_product, function(err, result1) {
            if(err)throw err;
            if(result1.length > 0){
...
router.get('/subscription', function(req, res, next) {
  ...
  res.render('_subscription');
         
});

router.get('/subscriptionpayments', function(req, res, next) {
  var amount = req.query.amount;
  var plan = req.query.plan;
  console.log('amount '+amount);
  res.render('_subscriptionpayments',{amount:amount, plan:plan});
